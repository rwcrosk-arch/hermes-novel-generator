...
### M0: Outline Generation (Chunked — Default Anti-Paralysis Pattern)

The single most common failure mode at this stage is **freeze/paralysis** when the assistant tries to mentally model and hand-type the entire `novel_state.yaml` (400+ lines) in one shot. This produces fragile YAML, slow execution, and zero tool calls. The chunked approach below is the **default and mandatory** method.

**Rule: Build the YAML programmatically in chunks. Never hand-type the full file in one response.**

```python
# Chunk 0: Skeleton — always start here
...
with open("novel_state.yaml", "w") as f:
    yaml.dump(state, f, default_flow_style=False, allow_unicode=True, sort_keys=False)
```

**Chunk order (genre-agnostic):**

| Chunk | Content | Max per tool call |
|-------|---------|-------------------|
| 0 | Skeleton (empty template) | 1 call |
| 1 | Characters (1-2 profiles per call) | 2 profiles |
| 2 | World rules + locations | 1 call |
| 3 | Premise, themes, narrative arc | 1 call |
| 4 | Chapter plan (2-3 chapters per call) | 3 chapters |
| 5 | Publishing metadata patch | 1 call |

**Rules to prevent freeze:**
1. **Use Python dicts + `yaml.dump()`, never hand-typed YAML.** This eliminates all indentation and quote-escaping errors.
2. **Load, modify, re-dump:**
   ```python
   import yaml
   state = yaml.safe_load(open("novel_state.yaml"))
   state["characters"].append(new_char_dict)
   with open("novel_state.yaml", "w") as f:
       yaml.dump(state, f, default_flow_style=False, allow_unicode=True, sort_keys=False)
   ```
3. **Never write more than ~60 lines of Python in a single `execute_code` call.** If a character profile is long, split across calls.
4. **Always validate after every chunk:** `yaml.safe_load()` on the written file. If it fails, use `execute_code` for line-level surgical fixes.
5. **Audit happens AFTER all chunks are complete.** Do not audit a partial outline. Generate all chunks first, then run one `delegate_task` audit.

**Anti-paralysis checklist before proceeding:**
- [ ] Skeleton (Chunk 0) exists and validates
- [ ] At least protagonist + 2 supporting characters in Chunk 1
- [ ] World rules and 3-5 locations in Chunk 2
- [ ] Premise + narrative arc in Chunk 3
- [ ] Full chapter plan (all chapters) in Chunk 4
- [ ] Author field populated in meta
- [ ] No em-dashes in any prose field
...
4. **Revise the outline** based on auditor feedback. Common issues: missing chapters for resolution, characters who disappear for 3+ chapters, dropped plot threads, romantic/found-family beats that are missing.
5. **Validate YAML** after every edit. Use `yaml.safe_load()` — even programmatic dumps can fail on circular references or non-serializable types.
